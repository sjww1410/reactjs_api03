...
import getColumns from './Credit_CardColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteCredit_Card, getCredit_Card} from "../../repo/credit_cardRepo";
/*
...
const Credit_CardTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteCredit_Card(oldData.credit_card_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Credit_Card Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getCredit_Card(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/credit_card/update/${rowData.credit_card_id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Credit_CardTable);
