...
import getColumns from './AddressColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteAddress, getAddress} from "../../repo/addressRepo";
/*
...
const AddressTable = (props) => {
    ...
    const handleRowDelete = (oldData, resolve) => {
        deleteAddress(oldData.address_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        ...
        data={async(query)=>
            {
                const res = await getAddress(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/address/update/${rowData.address_id}`,
                        user:rowData
                    })
                }
            },
