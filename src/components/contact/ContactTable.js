...
import {deleteContact, getContact} from "../../repo/contactRepo";
/*
...
const ContactTable = (props) => {
    ...
    const handleRowDelete = (oldData, resolve) => {
        deleteContact(oldData.contact_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        ...
        data={async(query)=>
            {
                const res = await getContact(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/contact/update/${rowData.contact_id}`,
                        user:rowData
                    })
                }
            },
