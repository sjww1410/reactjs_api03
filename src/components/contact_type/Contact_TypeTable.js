...
import getColumns from './Contact_TypeColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteContact_Type, getContact_Type} from "../../repo/contact_typeRepo";
/*
...
const Contact_TypeTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteContact_Type(oldData.contact_type_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Contact_Type Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getContact_Type(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/contact_type/update/${rowData.contact_type_id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Contact_TypeTable);
