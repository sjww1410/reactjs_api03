...
import {deleteTransaction, getTransaction} from "../../repo/transactionRepo";
/*
...
const TransactionTable = (props) => {
    ...
    const handleRowDelete = (oldData, resolve) => {
        deleteTransaction(oldData.transaction_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        ...
        data={async(query)=>
            {
                const res = await getTransaction(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/transaction/update/${rowData.transaction_id}`,
                        user:rowData
                    })
                }
            },
