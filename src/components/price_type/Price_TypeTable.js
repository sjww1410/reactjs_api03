...
import getColumns from './Price_TypeColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deletePrice_Type, getPrice_Type} from "../../repo/price_typeRepo";
/*
...
const Price_TypeTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deletePrice_Type(oldData.price_type_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Price_Type Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getPrice_Type(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/price_type/update/${rowData.price_type_id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Price_TypeTable);
