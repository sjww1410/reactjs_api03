...
import getColumns from './System_UserColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteSystem_User, getSystem_User} from "../../repo/system_userRepo";
/*
...
const System_UserTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteSystem_User(oldData.system_user_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="System_User Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getSystem_User(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/system_user/update/${rowData.system_user_id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(System_UserTable);
