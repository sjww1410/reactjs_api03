...
import getColumns from './TelephoneColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteTelephone, getTelephone} from "../../repo/telephoneRepo";
/*
...
const TelephoneTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteTelephone(oldData.telephone_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        ...
        data={async(query)=>
            {
                const res = await getTelephone(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/telephone/update/${rowData.telephone_id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(TelephoneTable);
