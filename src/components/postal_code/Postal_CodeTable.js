...
import getColumns from './Postal_CodeColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deletePostal_Code, getPostal_Code} from "../../repo/postal_codeRepo";
/*
...
const Postal_CodeTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deletePostal_Code(oldData.postal_code_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Postal_Code Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getPostal_Code(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/postal_code/update/${rowData.postal_code_id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Postal_CodeTable);
